...
/*  Created On : 17-март-2025 23:47:54 				*/
/*  DBMS       : PostgreSQL 						*/
/* ---------------------------------------------------- */
...
DROP SEQUENCE IF EXISTS m2m_rcp_igt_mri_id_seq
;
...
DROP TABLE IF EXISTS m2m_rcp_igt CASCADE
;
...
CREATE TABLE m2m_rcp_igt
(
	mri_id integer NOT NULL   DEFAULT NEXTVAL(('"m2m_rcp_igt_mri_id_seq"'::text)::regclass),
	mri_rcp_id integer NULL,
	mri_igt_id integer NULL
)
;
...
ALTER TABLE m2m_rcp_igt ADD CONSTRAINT "PK_m2m_recipe_type"
	PRIMARY KEY (mri_id)
;

CREATE INDEX "IXFK_m2m_recipe_type_ingredient_type" ON m2m_rcp_igt (mri_igt_id ASC)
;

CREATE INDEX "IXFK_m2m_recipe_type_recipe" ON m2m_rcp_igt (mri_rcp_id ASC)
;
...
ALTER TABLE m2m_rcp_igt ADD CONSTRAINT "FK_m2m_recipe_type_ingredient_type"
	FOREIGN KEY (mri_igt_id) REFERENCES ingredient_type (igt_id) ON DELETE No Action ON UPDATE No Action
;

ALTER TABLE m2m_rcp_igt ADD CONSTRAINT "FK_m2m_recipe_type_recipe"
	FOREIGN KEY (mri_rcp_id) REFERENCES recipe (rcp_id) ON DELETE No Action ON UPDATE No Action
;
...
CREATE SEQUENCE m2m_rcp_igt_mri_id_seq INCREMENT 1 START 1
;
